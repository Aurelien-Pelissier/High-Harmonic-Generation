function [z,Iq] = amplitude_ABSPMfree(dipq,ethaf,Pm,lp,profile,q,lambda1,znozzle,zmax,nres,gas)
%solving the maxwell equation to calculate the final harmonic amplitude
%with negligible absorption and perfect phasematching
...
%solving diff.eq.
sigma = 0;    %absorbtion cross section in m2
% sigma = 0;
E0 = 0;
...
if profile == 'gauss' | profile == 'squ_g'
    ...
    b = @(z)i*mu0*q*w1*c/2*Press(z,0,Pm,lp,profile,znozzle)/(T0*Kb)*100*(1-ionizf(ethaf,z,zmesh))*sqrt(dipoq(dipq,znozzle,zmesh));
